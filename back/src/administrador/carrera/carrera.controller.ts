...
import { carrera } from './carrera.interface'; @Controller('carrera')
export class CarreraController {     constructor(public carrera: CarreraService) { }
    @Get()
    ...
    @UseGuards(RolesGuard)
    @SetMetadata('roles', ['ADMINISTRADOR'])
    crear(@Body() bod: carrera) {       return this.carrera.crear(bod)
    }
    @Delete(":id")
    @UseGuards(RolesGuard)
    @SetMetadata('roles', ['ADMINISTRADOR'])
    eliminar(@Param("id") id :number ) {      return this.carrera.eliminar(id)     }
    @Put(":id")
    @UseGuards(RolesGuard)
    @SetMetadata('roles', ['ADMINISTRADOR'])
